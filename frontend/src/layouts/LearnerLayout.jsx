    ...
    >
      ...
      <header
        className={`shrink-0 z-50 border-b backdrop-blur-md transition-colors duration-300 relative ${
          isDarkMode
            ? 'bg-[#212631]/90 border-[#323846] text-[#f1f3f9]'
      ...
      >
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              >
                <span className="w-8 h-8 rounded-full bg-purple-600 text-white flex items-center justify-center font-mono text-xs font-bold shadow-md">
                  tz
                </span>

                <span
                  className={`tracking-normal font-bold text-xl ${
                    isDarkMode ? 'text-white' : 'text-slate-900'
                  }`}
                >
                  Thinkz<span className="text-purple-500 font-bold">.ai</span>
                </span>
              </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button
                  ...
                  className={`p-2.5 rounded-full transition-colors relative cursor-pointer flex items-center justify-center border ${
                    isDarkMode
                      ? 'bg-[#2a3040] hover:bg-[#32394c] text-[#94a3b8] hover:text-white border-[#3e4658]'
                      : 'bg-slate-100 hover:bg-slate-200 text-slate-600 border-slate-200'
                  }`}
              ...
              <button
                onClick={toggleTheme}
                className={`p-2 px-3.5 rounded-full transition-colors cursor-pointer text-xs font-medium border ${
                  isDarkMode
                    ? 'bg-[#2a3040] hover:bg-[#32394c] text-amber-300 border-[#3e4658]'
              ...
              {/* User */}
              <div
                className={`flex items-center gap-3 pl-2 sm:pl-4 sm:border-l ${
                  isDarkMode
                    ? 'sm:border-[#323846]'
                    : 'sm:border-slate-200'
                }`}
              >
                <span
                  className={`text-sm font-medium hidden sm:block ${
                    isDarkMode ? 'text-[#f1f3f9]' : 'text-slate-700'
                  }`}
                >
              ...
              <button
                ...
                className={`md:hidden p-2.5 rounded-full border focus:outline-none cursor-pointer ${
                  isDarkMode
                    ? 'bg-[#2a3040] border-[#3e4658] text-[#94a3b8]'
        ...
        {isMobileMenuOpen && (
          <div
            className={`md:hidden px-4 py-4 space-y-3 shadow-xl border-b ${
              isDarkMode
                ? 'bg-[#212631] text-[#f1f3f9] border-[#323846]'
          ...
          >
                ...
                <Link
                  ...
                  className={`block px-4 py-2 rounded-full text-sm font-medium ${
                    isActive
                      ...
                      : isDarkMode
                        ? 'text-[#94a3b8] hover:bg-[#2a3040] hover:text-white'
            ...
            <Link
              to="/learner/settings/notifications"
              onClick={handleLinkClick}
              className={`block px-4 py-2 rounded-full text-sm font-medium text-purple-400 ${
                isDarkMode ? 'hover:bg-[#2a3040]' : 'hover:bg-slate-100'
              }`}
            >
